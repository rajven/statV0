#==========================================================================
# Module: calamaris::calAxestype.pm
#
# Copyright
#
# calAxestype.pm Copyright (c) 2004 Michael Pophal based on
...
#
# Based on:
#          GD::Graph::axestype.pm,v 1.21 2000/04/15 08:59:36 mgjv
#          http://search.cpan.org/~mverb/GDGraph-1.43/Graph.pm
...
#
#   Name:
#       calamaris::calAxestype.pm
#
# $Id: calAxestype.pm,v 3.2 2004/09/15 21:02:13 cord Exp $
#
#--------------------------------------------------------------------------
...
package calamaris::calAxestype;

($axestype::VERSION) = '$Revision: 3.2 $' =~ /\s([\d.]+)/;
...
@calamaris::calAxestype::ISA = qw(GD::Graph);
...
sub draw_legend_marker # data_set_number, x, y
{
    ...
    my $ci = ($n<=2) ? $s->set_clr($s->pick_data_clr($n)) : $s->set_clr($s->_darken($s->_darken($s->pick_data_clr($n))));
